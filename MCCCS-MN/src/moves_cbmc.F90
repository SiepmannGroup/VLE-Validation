MODULE moves_cbmc
  ...
  use util_random,only:random,rotmat
...
!************************************************************
  subroutine rigrot(lnew,lterm,iskip,imol,imolty,ibox,wadd,first_bead,second_bead,mvtp)
    ...
    real::mat(3, 3),rbf,rxur(numax),ryur(numax),rzur(numax),length
       ...
       do ip = 1, ichoi
          if (lnew.or.ip.ne.1) then
            ...
            do j = igrow+1, iunit
                ...
                ry = ryur(j) - ryorig
                rz = rzur(j) - rzorig
                rxnw = mat(1, 1) * rx + mat(1, 2) * ry + mat(1, 3) * rz
                rynw = mat(2, 1) * rx + mat(2, 2) * ry + mat(2, 3) * rz
                rznw = mat(3, 1) * rx + mat(3, 2) * ry + mat(3, 3) * rz
